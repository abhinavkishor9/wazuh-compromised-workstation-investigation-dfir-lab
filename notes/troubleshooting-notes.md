...
- Baseline the endpoint before interpreting security changes.
- Pre-existing security weaknesses must not be attributed to later actions without evidence.
- Machine-account authentication events must be distinguished from human-user authentication.
- Network events must be matched to the correct process before being correlated.
- Wazuh provides centralized visibility but does not eliminate the need for local event validation.
- Multiple suspicious-looking activities do not automatically prove real compromise.
- Evidence gaps should be documented explicitly.
- A strong DFIR investigation distinguishes what happened, what appears suspicious, and what cannot be proven.
